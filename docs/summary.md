...
source/clean/poll_2024.py          Join with TSE metadata + match candidates
        ↓                          to TSE registry (politico_id, cpf, party)
source/clean/poll_sponsor_2024.py  Clean sponsor (contratante/pagante) data
        ↓
source/clean/poll_sponsor_2024_join.py
                                   Link sponsors to candidates via 4 routes:
...
| `build/clean/poll_2024.parquet` | Clean poll table with TSE metadata + candidate registry match |
| `build/clean/poll_sponsor_2024.parquet` | Sponsor table (one row per sponsor per poll) |
| `build/clean/poll_sponsor_2024_candidate.parquet` | Sponsors linked to PREFEITO candidates |
